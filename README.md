...
Each file is its own object in the `nuview-state-opendata` Amazon S3 bucket. Object keys are prefixed with a collection (which is composed from state/agency code and the remote sensing product type) and an acquisition project name (driven by state/agency internal naming conventions). Under the initial prefix the structures follow the USGS standards of collection (ex. the USGS [Lidar Base Specification](https://www.usgs.gov/ngp-standards-and-specifications/lidar-base-specification-online) for collections under the 3D Elevation Program).
